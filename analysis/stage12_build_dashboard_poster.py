...
<div class="grid">
  ...
  <div class="card span12">
    ...
    <p class="cap">AICc improvement from adding a level-shift dummy at each candidate break, vs. the no-intervention specification (manuscript Table 5). Orange = AICc favours a break; blue = it does not; grey "NT" = not testable (fewer than 2 post-break or 5 pre-break observations; the small number under "NT" shows how many post-break years the series actually has, out of the 2 required). Bold, asterisked cells (Δ&gt;2) are this paper's operational "decisive" threshold (Methods) -- a low bar, not a formal significance test. {n_currency_testable} of 21 series are even testable for the 2022 break -- most of the panel's WHO-GHO sources were last updated in 2021 or 2022, one or two years short of the 2 post-break years this test needs; this is a data-recency limit, not a modelling choice, and narrows as newer years are published. Hover any cell for the exact ΔAICc and post-break observation count. Testing rather than assuming away these breaks: a decisive currency-crisis break shifts the malaria 2030 forecast by a further {sb_malaria['currency_2022_pct_change_2030']:+.1f}% (larger than the order-selection effect above), and a decisive break at both candidate dates is found for the World Bank life-expectancy series -- a plausible partial explanation for its divergence from the WHO-GHO life-expectancy series, not a resolved causal account. Most series' break tests are underpowered on 2–5 post-break observations.</p>
...
function renderBreakheat() {{
  ...
  order.forEach(function(ind, i) {{
    ...
    [['covid', 0], ['currency', 1]].forEach(function(pair) {{
      ...
      if (val === null) {{
        svg += '<text x="' + (x + cellW / 2).toFixed(1) + '" y="' + (y + rowH / 2 - 1).toFixed(1) + '" font-size="8" text-anchor="middle" fill="#999">NT</text>';
        svg += '<text x="' + (x + cellW / 2).toFixed(1) + '" y="' + (y + rowH / 2 + 9).toFixed(1) + '" font-size="6.5" text-anchor="middle" fill="#aaa">(' + cellData.nPost + '/2y)</text>';
      }} else {{
